...
# Olympics Men's 100m Dash Over 100 Years

![](210315125224-usain-bolt-grinning-race.jpg){width=75%}

I scraped all available data for the times of every heat in the Men's 100m dash at the Olympics over nearly 100 years. 

Before we dive in to all times, I first extracted only the medal winners to see if there is a pattern there first. 
...
```{r}
oly_men_country_medals <- oly_men_100m %>%
  filter(Medal == c("Gold", "Silver", "Bronze")) %>%
  group_by(Athlete.Country) %>%
  count(Medal) %>%
  arrange(desc(n))

oly_men_country_medals$Medal <- factor(oly_men_country_medals$Medal, levels=c("Gold", "Silver", "Bronze"))
oly_men_country_medals$Athlete.Country <- as.factor(oly_men_country_medals$Athlete.Country)

oly_men_country_medals %>%
  ungroup() %>%
    gt() %>%
  tab_header(title = "Medal Count by Country",
             subtitle = "Men's 100m Dash") %>%
  tab_source_note(
    source_note = "Source: Wikipedia"
  ) %>%
    gtExtras::gt_theme_espn()
...
library(forcats)
 ...
 ggplot(oly_men_country_medals, aes(x = fct_reorder(Athlete.Country, n), y = n, fill = Medal)) +
  geom_col()+
  coord_flip()+
  theme(panel.background = element_blank(),
        plot.background = element_blank(),
  ...
  labs(y = "Number of Medals", x = " ",
       title = "Number of Medals Won by Country")+
  scale_fill_manual(values = c("#EBD739", "#B5BCC2", "#FFB48C"))
```

 
 

## Map of Medal Winning Countries
...
medal_color <- colorFactor(c("#FFB48C", "#EBD739", "#B5BCC2"), mens_100m_nation_medals$Rank)
